use crate::nvme::cmd as nvme_cmd;
use crate::nvme::queue::NvmeCommandQueue;
use crate::nvme::queue::NvmeCompletionQueue;
use crate::nvme::regs::NvmeArrayRegs;
use crate::nvme::regs::{NvmeRegs32, NvmeRegs64};
use crate::nvme::NUM_LBAS;
use crate::pci::utils::PciBarAddr;
use crate::println;
use alloc::boxed::Box;
use alloc::format;
use alloc::vec::Vec;
...
impl NvmeDevice {
    ...
    pub fn create_io_queues(&mut self) {
        for io_qid in 1..self.completion_queues.len() {
            ...
            println!(
                "  - Attempting to create I/O completion queue {} with phys {:x}",
                io_qid, ptr
            );
            {
                let qid = 0;
        ...
        for io_qid in 1..self.submission_queues.len() {
            ...
            println!(
                "  - Attempting to create I/O submission queue {} with phys {:x}",
                io_qid, ptr
            );
            {
                let qid = 0;
    ...
    pub fn configure_admin_queue(&self) {
        ...
        self.write_reg64(NvmeRegs64::ASQ, (asq.data.physical() as u64) << 12);
        self.write_reg64(NvmeRegs64::ACQ, (acq.data.physical() as u64) << 12);
