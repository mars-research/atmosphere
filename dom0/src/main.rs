...
fn test_new_send_no_wait() {
    unsafe {
        ...
        let end = _rdtsc();
        log::info!(
            "send no wait cycle per syscall {:?}",
            (end - start) as usize / iter
        );
    }
}
...
fn test_new_endpoint() {
    ...
    unsafe {
        ...
        let end = _rdtsc();
        log::info!(
            "new endpoint cycle per syscall {:?}",
            (end - start) as usize / iter
        );
    }
}
...
fn test_mmap() {
    ...
    unsafe {
        let start = _rdtsc();
        for i in 0..iter {
            let error_code = asys::sys_mmap(
                0xA000000000 + 4096 * i,
                0x0000_0000_0000_0002u64 as usize,
                1,
            );
            if error_code != 0 {
