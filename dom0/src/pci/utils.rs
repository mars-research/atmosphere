...
use super::header::{read_config_space, PciDeviceHeader};
use super::println;
use super::PciHeaderType;
use alloc::vec::Vec;
...
impl fmt::Display for PciBarAddr {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "base = 0x{:08X}, len = 0x{:04X}", self.base, self.size)
    }
}
...
pub fn pci_read(pci: &PciAddress, offset: u8) -> u32 {
    ...
    unsafe {
        ...
        out("eax") value, options(nostack, nomem, preserves_flags));
    }
    value
}
