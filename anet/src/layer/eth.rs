use alloc::collections::VecDeque;
use pnet::{packet::ethernet::EtherType, util::MacAddr};

pub const ETHER_HDR_LEN: usize = 14;

const IPV4_ETHER_TYPE: u16 = 0x800;

use crate::{packet::UdpPacketRepr, util::MacAddress};

pub struct EthernetLayer {
    endpoint: MacAddr,
}

impl EthernetLayer {
    pub fn new(endpoint: MacAddr) -> Self {
        Self { endpoint }
    }

    pub fn send_packet<F>(
        &self,
        buf: &mut [u8],
        dmac: MacAddress,
        ether_type: EtherType,
        f: F,
    ) -> Result<usize, ()>
    where
        F: FnOnce(&mut [u8]) -> usize,
    {
        todo!()
        // buf[0..6].copy_from_slice(&self.endpoint.0);
        // buf[6..12].copy_from_slice(&dmac.0);

        // buf[12..14].copy_from_slice(&(ether_type as u16).to_be_bytes());

        // let payload_len = f(&mut buf[14..1504]);

        // Ok(payload_len + ETHER_HDR_LEN)
    }

    pub fn recv_packet<F>(&self, buf: &[u8], f: F) -> Result<MacAddress, ()>
    where
        F: FnOnce(MacAddress, &[u8]) -> (),
    {
        todo!()
        // let mut mac_addr = MacAddress::default();

        // mac_addr = MacAddress::from_slice(&buf[0..6]);
        // f(mac_addr, &buf[14..]);

        // Ok(mac_addr)
    }

    pub fn prepare_udp_batch(&self, dest: MacAddr, packets: &mut VecDeque<UdpPacketRepr>) {
        for packet in packets {
            packet.set_eth_packet(|mut eth| {
                eth.set_destination(dest);
                eth.set_source(self.endpoint);
                eth.set_ethertype(EtherType::new(IPV4_ETHER_TYPE));
            });
        }
    }
}
