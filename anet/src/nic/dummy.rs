...
use crate::{util::{RawPacket, flip_eth_hdr, flip_ip_hdr, flip_udp_hdr}, RpcResult};
...
impl DummyNic {
    pub fn new() -> Self {
        Self { echo_queue: Mutex::new(VecDeque::new()) }
    }
}
...
impl Net for DummyNic {
    ...
    fn submit_batch(&self, send_bufs: &mut VecDeque<RawPacket>, return_bufs: &mut VecDeque<RawPacket>) -> RpcResult<usize> {
    ...
    fn poll(&self, bufs: &mut VecDeque<RawPacket>, recvd_bufs: &mut VecDeque<RawPacket>) -> crate::RpcResult<usize> {
        let mut g = self.echo_queue.lock();

        let num_pkts = core::cmp::min(g.len(), bufs.len());

        g.drain(0..num_pkts).zip(bufs.drain(0..num_pkts)).for_each(|(pkt, mut recv_buf)| {
...
mod test {
    ...
    use alloc::collections::VecDeque;
    use pnet::packet::{ethernet::EthernetPacket, ipv4::Ipv4Packet, Packet, udp::UdpPacket, FromPacket};
    ...
    pub fn test_dummy_nic_send() {
        ...
        assert_eq!(sent_ip.get_next_level_protocol(), recvd_ip.get_next_level_protocol());
