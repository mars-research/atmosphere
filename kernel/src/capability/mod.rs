...
//!
//! Currently we do not implement guards in CNodes.
...
impl CapPointerResolution {
    ...
    /// Extracts a number of bits from the not-yet-resolved end of MSB.
...
impl CNode {
    /// Constructs a CNode with a given radix.
    ...
    /// Resolves a CapPointer and returns a pointer to the CSlot.
    fn resolve_ptr(&self, mut resolution: CapPointerResolution) -> Option<*const CSlot> {
    ...
    /// Inserts a capability into the first empty CSlot, returning its index and address.
...
impl Capability {
    ...
    pub fn capability_type(&self) -> CapType {
        match self.data {
