...
impl ProcessManager {
    pub fn new_proc_with_endpoint_iommu(
        &mut self,
        ...
        requires
            ...
            forall|p_ptr_i: ProcPtr|
                #![trigger old(self).proc_dom().contains(p_ptr_i) ]
                old(self).proc_dom().contains(p_ptr_i) && old(self).get_proc(p_ptr_i).ioid.is_some()
                    ==> old(self).get_proc(p_ptr_i).ioid.unwrap() != new_ioid,
            page_perm_1@.is_init(),
        ...
        ensures
            ...
            self.get_proc(page_ptr_1).ioid.is_Some(),
            self.get_proc(page_ptr_1).ioid.unwrap() == new_ioid,
