...
pub closed spec fn syscall_send_pages_wait_spec(old:Kernel, new:Kernel, cpu_id:CPUID, endpoint_index: EndpointIdx, va: VAddr, range: usize) -> bool
{
    if !old.wf() || !new.wf()
    {
        false
...
impl Kernel {
    pub fn syscall_send_pages_wait(&mut self, cpu_id:CPUID, pt_regs: &mut Registers, endpoint_index: EndpointIdx, va: VAddr, range: usize) -> (ret: SyscallReturnStruct)
        requires
            old(self).wf(),
        ensures
            self.wf(),
            syscall_send_pages_wait_spec(*old(self), *self, cpu_id, endpoint_index,va, range),
    {
        ...
        if self.proc_man.get_endpoint_state_by_endpoint_ptr(target_endpoint_ptr) == SEND {
            ...
            if self.proc_man.get_endpoint_len_by_endpoint_ptr(target_endpoint_ptr) == 0{
                ...
                {                   
                ...
                }else if self.kernel_page_sharing_helper(sender_pcid, receiver_pcid, perm_bits, va, receiver_ipc_payload.page_payload.unwrap().0, range) == true{
                   ...
                   // assert(kernel_page_sharing_spec_helper(*old(self),sender_pcid,receiver_pcid, va, receiver_ipc_payload.page_payload.unwrap().0, range));
                    ...
                    self.cpu_list.set_current_thread(cpu_id,Some(new_thread_ptr));
                    self.kernel_map_pagetable_range_page_to_pagetable(sender_pcid, receiver_pcid, va, receiver_ipc_payload.page_payload.unwrap().0, perm_bits,range);
