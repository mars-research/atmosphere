...
impl Kernel {
    pub fn syscall_send_empty_wait(&mut self, cpu_id:CPUID, pt_regs: &mut Registers, endpoint_index: EndpointIdx) -> (ret: (SyscallReturnStruct,Option<(Pcid,usize,ThreadPtr)>))
        requires
            old(self).wf(),
        ensures
            self.wf(),
