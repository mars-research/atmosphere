...
pub closed spec fn syscall_receive_message_wait_spec(old:Kernel, new:Kernel, cpu_id:CPUID, endpoint_index: EndpointIdx, message_va: VAddr, length:usize) -> bool
{
    if !old.wf() || !new.wf()
    {
        false
...
impl Kernel {
    pub fn syscall_receive_message_wait(&mut self, cpu_id:CPUID, pt_regs: &mut Registers, endpoint_index: EndpointIdx, message_va: VAddr, length:usize) -> (ret: SyscallReturnStruct)
        requires
            old(self).wf(),
        ensures
            self.wf(),
            syscall_receive_message_wait_spec(*old(self), *self, cpu_id, endpoint_index,message_va,length),
    {
        ...
        if self.proc_man.get_endpoint_state_by_endpoint_ptr(target_endpoint_ptr) == RECEIVE {
            ...
            if self.proc_man.get_endpoint_len_by_endpoint_ptr(target_endpoint_ptr) == 0{
                ...
                }else if (va_valid(message_va) == false) || (va_valid(sender_ipc_payload.message.unwrap().0) == false) || (length != sender_ipc_payload.message.unwrap().1)
                            || (length>4096) || (length<=0)
                {
                    ...
                    return SyscallReturnStruct::new(MESSAGE_INVALID,new_pcid,new_cr3,new_thread_ptr);
                }else{
                    let receiver_pa_op = self.mmu_man.mmu_get_va_entry_by_pcid(pcid,message_va);
                    let sender_pa_op = self.mmu_man.mmu_get_va_entry_by_pcid(new_pcid,sender_ipc_payload.message.unwrap().0);
