...
impl Kernel {
    pub fn syscall_send_endpoint_wait(&mut self, cpu_id:CPUID, pt_regs: &mut Registers, endpoint_index: EndpointIdx, share_endpoint_index: EndpointIdx) -> (ret: SyscallReturnStruct)
        requires
            old(self).wf(),
        ensures
            self.wf(),
            syscall_send_endpoint_wait_spec(*old(self), *self, cpu_id, endpoint_index,share_endpoint_index),
    {
