...
impl DynLinkedlist{
    ...
    pub fn remove(&mut self, rf:usize)
        ...
        ensures
            self.wf(),            
            self@ =~= old(self)@.subrange(0, old(self)@.index_of(old(self).node_ref_resolve(rf))) + old(self)@.subrange(old(self)@.index_of(old(self).node_ref_resolve(rf)) + 1, old(self)@.len() as int),
            ...
            self@.contains(old(self).node_ref_resolve(rf)) == false,
    {
        ...
        assert((forall|i: int| #![auto] 0 <= i < self.len() ==> self.spec_seq@[i] =~= self.get_node_by_dyn_index(self.value_list@[i]).value));
        assert(self.spec_seq@[rf_value_list_index@] =~= self.get_node_by_dyn_index(self.value_list@[rf_value_list_index@]).value);
        assert(rf_value_list_index@ == old(self)@.index_of(old(self).node_ref_resolve(rf)));

        if rf_ptr == self.value_head.ptr && rf_index == self.value_head.index{
            assert(self.node_ref_resolve(rf) == self@[0]);
            // assert(dyn_index_to_usize(&self.value_list@[0]) == rf);
            self.pop();
            return;

        }else if rf_ptr == self.value_tail.ptr && rf_index == self.value_tail.index && self.len() == self.size() {

            assert(self.free_list@.len() == 0);
            assert(self.value_list@.len() > 1);

            assert(self.value_list@[self.value_list@.len() - 1] == self.value_tail);
            assert(self.next_value_node_of(self.value_list@.len() - 1).ptr == 0 && self.next_value_node_of(self.value_list@.len() - 1).index == 0);
            assert(self.get_node_by_dyn_index(self.value_tail).next.ptr == 0 && self.get_node_by_dyn_index(self.value_tail).next.index == 0);
            ...
            dyn_array_set_next(&PPtr::<DynArray>::from_usize(prev_value_ptr), &mut prev_value_array_perm, 0,0,prev_value_index);
            proof{
            ...
            proof{
                self.value_list@ = self.value_list@.subrange(0, rf_value_list_index@);
                self.spec_seq@ = self.spec_seq@.subrange(0, rf_value_list_index@);
            }
            ...
            self.value_tail.ptr = prev_value_ptr;
            self.value_tail.index = prev_value_index;
            ...
            assert(self.value_list@ =~= old(self).value_list@.subrange(0, rf_value_list_index@));
            ...
            assert(forall|i: int| #![auto] 0 <= i < self.value_list@.len() - 1 ==> self.get_node_by_dyn_index(self.value_list@[i as int]).next == old(self).get_node_by_dyn_index(old(self).value_list@[(i as int)]).next);
            assert(forall|i: int| #![auto] 0 <= i < self.value_list@.len() - 1 ==> self.next_value_node_of(i) == old(self).next_value_node_of(i));
            ...
            assert(forall|i: int| #![auto] 0 <= i < self.value_list@.len() ==> self.get_node_by_dyn_index(self.value_list@[i as int]).prev == old(self).get_node_by_dyn_index(old(self).value_list@[(i as int)]).prev);
    ...
    pub fn pop(&mut self) -> (ret:usize)
        ...
        ensures
            ...
            self.size() == old(self).size(),
            self.len() == old(self).len() - 1,
            forall|rf:usize| old(self).node_ref_valid(rf) && old(self).node_ref_resolve(rf) != ret ==> self.node_ref_valid(rf),
            forall|rf:usize| old(self).node_ref_valid(rf) && old(self).node_ref_resolve(rf) != ret ==> self.node_ref_resolve(rf) == old(self).node_ref_resolve(rf),
